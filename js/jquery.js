...
(function() {
	...
	jQuery.fn.offset = function() {
		var left = 0,
			...
			results;
			if (elem) {
				var browser = jQuery.browser, parent = elem.parentNode,
				offsetChild = elem,
			...
			if (elem.getBoundingClientRect) {
				...
				while (offsetParent) {
					...
					if (browser.mozilla && !/^t(able|d|h)$/i.test(offsetParent.tagName) || browser.safari && !safari2) border(offsetParent);
				...
				while (parent && parent.tagName && !/^body|html$/i.test(parent.tagName)) {
					...
					if (browser.mozilla && jQuery.css(parent, "overflow") != "visible") border(parent);
					parent = parent.parentNode;
				}
				if ((safari2 && (fixed || jQuery.css(offsetChild, "position") == "absolute")) || (browser.mozilla && jQuery.css(offsetChild, "position") != "absolute")) add(-doc.body.offsetLeft, -doc.body.offsetTop);
